...
public:
    ...
    void run(const int refinements, ConvergenceTable &convergence_table, const double final_time ) {
        ...
        while( current_time < final_time ) {
            ...
            assemble_system1( true, current_time);
            this->solve();
            assemble_system1( false, current_time );
            ...
            assemble_system2( true, current_time );
            this->solve();
            assemble_system2( false, current_time );
            ...
            assemble_system3( true, current_time );
            this->solve();
            assemble_system3( false, current_time );
...
//                this->solution_local[i] = this->solution_local[i] + 3 * ( - this->solution_local_mid[i] + this->solution_local_mid2[i] );
//                this->solution_local[i] = 2 * this->solution_local_mid[i] - this->solution_local[i];
                this->solution_local[i] = -this->solution_local[i] + 4 * this->solution_local_mid[i] - 6 * this->solution_local_mid2[i] + 4 * this->solution_local_mid3[i];
            }
            current_time += dt;   /// using dt = h^{(k+1)/2}/2x
        }
        cout << "   -> Current time:  " << current_time << endl << endl;
...
//        dt = pow( pow(2.0, -7 ), (this->degree+1.0)/2 )/2;
        dt = h_mesh/(this->degree + 1); //(this->degree + 1.0); // pow( h_mesh, (this->degree + 1.0)/2 )/2; /// using dt = h^{(k+1)/2}
        ...
        this->solution_local_mid2.reinit(this->dof_handler_local.n_dofs());
        this->solution_local_mid3.reinit(this->dof_handler_local.n_dofs());
    ...
    void assemble_system1(bool globalProblem , const double current_time) {
...
//                loc_cell->set_dof_values(U_vector, this->solution_local);       // using dt = h^{k+1}
    ...
    void assemble_system2(bool globalProblem , const double current_time) {
...
//        vector<double>          prev_sigma_div          ( n_q_points );
        ...
        for (const auto &cell : this->dof_handler.active_cell_iterators()) {
            ...
            source_term.value_list(fe_values_local.get_quadrature_points(), st_values);
            fe_values_local[concentration].get_function_values( this->solution_local_mid , prev_solution);
...
//                            F_vector(i) += dt * fmax(-u_vel * normal,0) * phi_i_c * mult_values[q] * JxW; /// Oikawa
                ...
                A_matrix.vmult(U_vector, F_vector, false);    // A^{-1} * F
//                loc_cell->set_dof_values(U_vector, this->solution_local);       // using dt = h^{k+1}
                loc_cell->set_dof_values(U_vector, this->solution_local_mid2);   // using dt = h^{(k+1)/2}/2
            }
        }
    }

    void assemble_system3(bool globalProblem , const double current_time) {
...
//        vector<double>          prev_sigma_div          ( n_q_points );
        ...
        for (const auto &cell : this->dof_handler.active_cell_iterators()) {
            ...
            source_term.value_list(fe_values_local.get_quadrature_points(), st_values);
            fe_values_local[concentration].get_function_values( this->solution_local_mid2 , prev_solution);
...
//                            F_vector(i) += dt * fmax(-u_vel * normal,0) * phi_i_c * mult_values[q] * JxW; /// Oikawa
                ...
                A_matrix.vmult(U_vector, F_vector, false);    // A^{-1} * F
//                loc_cell->set_dof_values(U_vector, this->solution_local);       // using dt = h^{k+1}
                loc_cell->set_dof_values(U_vector, this->solution_local_mid3);   // using dt = h^{(k+1)/2}/2
            }
        }
    }
...
//        VectorTools::integrate_difference(this->dof_handler_local, this->solution_local, exact_sol, cellwise_errors, this->quadrature, VectorTools::L2_norm, &vector_mask);
        ...
        VectorTools::integrate_difference(this->dof_handler_local, this->solution_local_mid2, exact_sol, cellwise_errors, this->quadrature, VectorTools::L2_norm, &vector_mask);
...
int main () {
    ...
    for (int refinement = 2; refinement <= 5; ++refinement) {
//        double final_time = pow( pow(2.0, -refinement ), (degree+1.0)/2 ); /// Just one iteration each mesh
    ...
    show_convergence(convergence_table , dim, "TransVander" );
}
